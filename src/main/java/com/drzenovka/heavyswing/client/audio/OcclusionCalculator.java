...
public class OcclusionCalculator {
    ...
    private static class PosEntry { final float coeff; final int expires; PosEntry(float c,int e){coeff=c;expires=e;} }
    private static final LinkedHashMap<Long, PosEntry> POS_CACHE = new LinkedHashMap<Long, PosEntry>(POS_CACHE_SIZE,0.75f,true){
        @Override protected boolean removeEldestEntry(Map.Entry<Long, PosEntry> eldest){ return size() > POS_CACHE_SIZE; }
    };
